...
public class showSeniors {
    ...
    public static Object[][] getSeniorInfo(Connection connection) {
        ...
        try {
            ...
            String seniorCountQuery = "SELECT COUNT(*) FROM Students WHERE grade_year = 12";
            countPS = studentConnection.prepareStatement(seniorCountQuery);
    ...
    public static Object[][] getSeniorService(Connection connection) {
        PreparedStatement servicePS = null;
        ResultSet serviceResults = null;
        ArrayList<Object[]> serviceData = new ArrayList<>();

        try {
            Object[][] seniorInfo = getSeniorInfo(connection);

            if (seniorInfo.length == 0) {
                return new Object[0][0];
            }

            // Hash map for quick and easy look up
            Map<Integer, String[]> seniorMap = new HashMap<>();
            StringBuilder idList = new StringBuilder();
            boolean first = true;

            for (Object[] student : seniorInfo) {
                int studentID = (Integer) student[0];
                String firstName = (String) student[1];
                String lastName = (String) student[2];

                seniorMap.put(studentID, new String[] {firstName, lastName});
            ...
            while (serviceResults.next()) {
                ...
                if (seniorMap.containsKey(studentID)) {
                    String[] studentInfo = seniorMap.get(studentID);
